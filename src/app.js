...
$(document).ready(function() {
  ...
  const getMovies = () => {
    const movieURL =
      "https://api.themoviedb.org/3/genre/27/movies?api_key=2434d246ec60c162a86db597467ef4ed&language=en-US&include_adult=false&sort_by-created_at.acs";
    fetch(movieURL)
      .then(res => res.json())
      .then(payload => {
  ...
  const creatPosters = () => {
    for (let i = 0; i < moviesList.length; i++) {
      ...
      var divThumbnail = $("<div>")
        ...
        .append(
          $("<img>")
            .attr("src", "https://image.tmbd.org/t/p/w500" + movie.poster_path)
  ...
  getMovies();
  //Take Poster URL's and create an img tag from each one,
  // gonna need to be it's own box, that box will have clo-md-4
});
